...
loop.run_until_complete(asyncio.gather(
  ...
  expect_response(anne, '!startimmediate', game,
    '[game] start(@anne, @bob, @carl, @david, @elsa, @frank, @george, @harry, @ignacio) list_roles(insomniac, clone, drunk, troublemaker, thief, villager, seer, wolf, minion, villager, villager, wolf) ',
...
loop.run_until_complete(asyncio.gather(
  ...
  expect_response(ignacio, '!vote david', game, '[game] vote_success(@ignacio, @david) remind_unvote(!unvote) ', '[game] vote_countdown({}) '.format(core.VOTE_COUNTDOWN) ),
  ...
  expect_response(george, '!vote david', game, '[game] vote_success(@george, @david) remind_unvote(!unvote) ', '[game] vote_result(vote_item(@frank, 1) \nvote_item(@anne, 1) \nvote_item(@harry, 3) \nvote_item(@david, 1) \nvote_item(@elsa, 3) ) ', '[game] no_lynch ', '[game] winners(@carl, @bob, @elsa, @harry) ', '[game] reveal_all(anne:insomniac\ncarl:wolf\nbob:wolf\ndavid:troublemaker\nelsa:minion\nfrank:villager\ngeorge:thief\nharry:wolf\nignacio:seer) \nexcess_roles(drunk, villager, villager) ' ),
  expect_response(anne, '!load _test', game, '[game] confirm(@anne) load_success(_test) '),
))
...
loop.run_until_complete(asyncio.gather(
  ...
  expect_response(ignacio, '!votenolynch', game, '[game] no_vote_success(@ignacio) remind_unvote(!unvote) ', '[game] vote_result(vote_item(no_lynch , 9) ) ', '[game] no_lynch ', '[game] winners(@carl, @bob, @elsa, @harry) ', '[game] reveal_all(anne:insomniac\ncarl:wolf\nbob:wolf\ndavid:troublemaker\nelsa:minion\nfrank:villager\ngeorge:thief\nharry:wolf\nignacio:seer) \nexcess_roles(drunk, villager, villager) ' ),
))
